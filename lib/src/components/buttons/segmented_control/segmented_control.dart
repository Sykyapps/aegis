...
/// callback and rebuild the segmented control with a new [currentSegment] to update
...
///
/// {@tool dartpad}
/// This example shows a [SkSegmentedControl] with an enum type.
///
...
class SkSegmentedControl<T> extends StatefulWidget {
  ...
  /// is selected.
  ///
  /// The [currentSegment] is the currently selected value for the segmented control.
  /// If no [currentSegment] is provided, or the [currentSegment] is null, no widget will
  /// appear as selected. The [currentSegment] must be either null or one of the keys
  /// in the [children] map.
  SkSegmentedControl({
    super.key,
    ...
    this.higlightedTextColor = AegisColors.neutral500,
        ...
        assert(
          currentSegment == null || children.keys.contains(currentSegment),
  ...
  /// widget rebuilds the segmented control with the new [currentSegment].
  ///
  /// The callback provided to [onValueChanged] should update the state of
  /// the parent [StatefulWidget] using the [State.setState] method, so that
  /// the parent gets rebuilt; for example:
...
class _SegmentedControlState<T> extends State<SkSegmentedControl<T>>
    with TickerProviderStateMixin<SkSegmentedControl<T>> {
  ...
  void initState() {
    ...
    highlighted = widget.currentSegment;
  }

  @override
  void didUpdateWidget(SkSegmentedControl<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
    ...
    if (!isThumbDragging && highlighted != widget.currentSegment) {
  ...
  void onTapUp(TapUpDetails details) {
    ...
    if (segment != widget.currentSegment) {
  ...
  void onEnd(DragEndDetails details) {
    ...
    if (isThumbDragging) {
      ...
      if (highlighted != widget.currentSegment) {
    ...
    for (final MapEntry<T, Widget> entry in widget.children.entries) {
      ...
      children.add(
        Semantics(
          ...
          selected: widget.currentSegment == entry.key,
          child: MouseRegion(
...
class _RenderSegmentedControl<T> extends RenderBox
  ...
  double computeMinIntrinsicHeight(double width) {
    RenderBox? child = firstChild;
    double maxMinChildHeight = _kMinSegmentedControlHeight.r;
    while (child != null) {
  ...
  double computeMaxIntrinsicHeight(double width) {
    RenderBox? child = firstChild;
    double maxMaxChildHeight = _kMinSegmentedControlHeight.r;
    while (child != null) {
  ...
  Size _calculateChildSize(BoxConstraints constraints) {
    ...
    double maxHeight = _kMinSegmentedControlHeight.r;
    RenderBox? child = firstChild;
    while (child != null) {
      childWidth = math.max(
          childWidth,
  ...
  void _paintThumb(PaintingContext context, Offset offset, Rect thumbRect) {
    ...
    const List<BoxShadow> thumbShadow = <BoxShadow>[
      BoxShadow(
        ...
        color: Color.fromRGBO(48, 49, 53, .16),
        // rgba(48, 49, 53, 0.16)
        offset: Offset(0, 2),
        blurRadius: 8,
      ),
